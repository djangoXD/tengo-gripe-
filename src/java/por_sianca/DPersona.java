...
public class DPersona implements Operaciones{
    ...
    public String eliminar(Object o) {
        ...
        try{
            pst=(PreparedStatement) db.conexion.prepareStatement(sql);
            pst.setString(1, p.getId());            
            boolean filas=pst.execute();
            res="Se eliminarons "+ filas + "nuevo elemento";
        } catch (SQLException ex) {
            res=ex.getMessage()+" "+sql;
        }
        return res;
    ...
    public String modificar(Object o) {
          CPersona p=(CPersona)o;
        String sql="update persona set nombre=? ,apellidos=?,fecha_nacimiento=?,direccion=?,tipo_documento=?,nro_documento=?,email=?,sexo=? where id=?";
        db.con();
        PreparedStatement pst;
        String res="";
        try{
